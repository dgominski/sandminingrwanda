...
class TransUnetModel(BaseModel, torch.nn.Module):
    def __init__(self, opt, mode):
        BaseModel.__init__(self, opt)
        torch.nn.Module.__init__(self)
        self.mode = mode
        print(f"## TransUNet pretrained on ImageNet")
        self.loss_names = ['pixel', 'region', 'total']
        self.scalar_names = ["jaccard", "dice"]
        self.model_names = ['transformer', 'decoder', 'segmentation_head']
        self.visual_names = ["visual_target", "visual_pred"]

        # download a TransUnet checkpoint from there https://github.com/Beckschen/TransUNet and indicate the path to the npz file below, then uncomment
        config = CONFIGS["R50-ViT-B_16"]
        transunet = VisionTransformer(config, img_size=opt.imsize, num_classes=1).to(self.device)
        # transunet.load_from(weights=np.load(TRANSUNET_CKPT))
        self.transformer = transunet.transformer
        self.decoder = transunet.decoder
        self.segmentation_head = nn.Sequential(
            nn.Conv2d(16, 16, kernel_size=(3, 3), padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(16, 2, kernel_size=(1, 1), stride=(1, 1), padding=0, bias=True)).to(self.device)

        self.criterion_region = TverskyLoss(mode="binary", from_logits=True, beta=.7, alpha=.3)
        self.criterion_pixel = CrossEntropyLoss()

        self.optimizers["main"] = torch.optim.Adam(list(self.transformer.parameters()) + list(self.decoder.parameters()) + list(self.segmentation_head.parameters()), lr=opt.lr, betas=(0.9, 0.999), weight_decay=1e-4)

    def forward(self, img):
        x, attn_weights, features = self.transformer(img.to(self.device))  # (B, n_patch, hidden)
        feats = self.decoder(x, features)
        self.heatmap = self.segmentation_head(feats)
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       ...
                                       kernel_size=patch_size, # conv2d use the kernel_size make it to linear projection
                                       stride=patch_size)
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        encoded, attn_weights = self.encoder(embedding_output)  # (B, n_patch, hidden) #hidden feature after TranLayers
...
if __name__ == "__main__":
    ...
    for i in range(20):
        ...
        tf = transforms.Normalize(mean=(96, 103, 86),
                             std=(43, 40, 39))
        norm_data = tf(data)
        probs, mask = model(norm_data, norm_data)
        display(data[0], hold=True)
        display(probs[:, 0], hold=True, to_int=False, title="background")
